...
peer:null,conns:{},rtimers:{},backoff:{},syncPeer:null,syncPeerId:null,syncConns:{},syncGen:0,leaderSeenAt:0,leaderCheckTimer:null,
...
S.peer.on("open",e=>{if(gen!==S.peerGen)return;S.peerReady=!0;S.signalingOk=!0;S.peerRestartBackoff=3e3;S.peerId=e;S.receiverMode=e===S.accountId?"leader":"sync";if(S.receiverMode==="leader")S.leaderSeenAt=Date.now();updateReceiverStatus();updateTopBar();renderDeviceList();if(S.receiverMode==="leader"){reconnectAll()}else{setTimeout(()=>{if(gen===S.peerGen)connectTo(S.accountId,!0)},400)}});
...
S.peer.on("error",e=>{if(gen!==S.peerGen)return;console.warn("[NET]",e.type,e.message);if(e.type==="unavailable-id"){let next=S.receiverMode==="leader"?"sync":S.receiverMode;S.peerRestartBackoff=Math.min(S.peerRestartBackoff*1.5,3e4);schedulePeerRestart(next,700+Math.random()*S.peerRestartBackoff);checkForAccountLeader(1800)}else if(["network","server-error","socket-error","socket-closed"].includes(e.type)){S.peerReady=!1;S.signalingOk=!1;document.getElementById("sig-dot").className="sdot-sm";document.getElementById("sig-lbl").textContent="offline";S.peerRestartBackoff=Math.min(S.peerRestartBackoff*1.5,3e4);schedulePeerRestart(S.receiverMode,S.peerRestartBackoff)}else if(e.type==="peer-unavailable"){let m=e.message?.match(/Could not connect to peer ([^\s]+)/);if(m){if(m[1]===S.accountId&&S.receiverMode==="sync")schedulePromotion();else schedRec(m[1],15e3)}}})}
...
function checkForAccountLeader(delay=1200){clearTimeout(S.leaderCheckTimer);S.leaderCheckTimer=setTimeout(()=>{if(S.receiverMode==="leader"&&S.peerReady){S.leaderSeenAt=Date.now();return}connectTo(S.accountId,!0);connectKnownSyncDevices();syncCast({type:"leader-query",from:syncHello(),ts:Date.now()});setTimeout(electOrAskLeader,2500)},delay)}
function electOrAskLeader(){if((S.receiverMode==="leader"&&S.peerReady)||S.conns[S.accountId]?.open||Date.now()-S.leaderSeenAt<8000)return;let devices=[{syncPeerId:S.syncPeerId,deviceId:S.deviceId,online:true},...Object.values(S.linkedDevices).filter(d=>d.sameAccount)];devices=devices.filter(d=>d.syncPeerId).sort((a,b)=>a.syncPeerId.localeCompare(b.syncPeerId));let chosen=devices[0];if(!chosen||chosen.syncPeerId===S.syncPeerId)_startPeer("leader");else{let c=getDeviceConn(chosen.syncPeerId);if(c?.open)send(c,{type:"leader-request",targetSyncPeerId:chosen.syncPeerId,from:syncHello(),ts:Date.now()})}}
...
function installWakeRecoveryHooks(){if(installWakeRecoveryHooks.done)return;installWakeRecoveryHooks.done=true;let last=Date.now();setInterval(()=>{let now=Date.now();if(now-last>15000&&S.peer&&!S.peer.destroyed){clearTimeout(S.peerRestartTimer);schedulePeerRestart(S.receiverMode,800)}last=now},5000);window.addEventListener("online",()=>schedulePeerRestart(S.receiverMode,300));window.addEventListener("offline",()=>{S.peerReady=!1;S.signalingOk=!1;document.getElementById("sig-lbl").textContent="offline"})}
function requestTakeover(){if(S.receiverMode!=="sync"||Date.now()<S.takeoverUntil)return;if(!S.syncReady){clearTimeout(S.syncWaitTimer);S.syncWaitTimer=setTimeout(requestTakeover,1200);return}S.takeoverUntil=Date.now()+8000;let c=S.conns[S.accountId];if(c?.open)send(c,{type:"takeover-request",deviceId:S.deviceId,name:S.myName});else{syncCast({type:"leader-query",from:syncHello(),ts:Date.now()});checkForAccountLeader(500)}}
...
async function handleData(t,n){
  switch(n.type){
    ...
    case "leader-request":{
      if(n.from?.accountId!==S.accountId)break;
      rememberLinkedDevice({...n.from,sameAccount:true,online:true});
      if(n.targetSyncPeerId===S.syncPeerId&&S.receiverMode!=="leader")setTimeout(()=>_startPeer("leader"),300+Math.random()*700);
      break;
    }
    case "takeover-request":{
      ...
      setTimeout(()=>_startPeer("sync"),1200);
      break;
    }
    case "takeover-grant":{
      if(!isSameAccount(t))break;
      S.syncReady=true;setTimeout(()=>_startPeer("leader"),250);
      break;
    }
    case "dm":{
      let f=S.friends[t];
